-- CreateTable
CREATE TABLE "Account" (
    "id" TEXT NOT NULL,
    "userId" TEXT NOT NULL,
    "type" TEXT NOT NULL,
    "provider" TEXT NOT NULL,
    "providerAccountId" TEXT NOT NULL,
    "refresh_token" TEXT,
    "access_token" TEXT,
    "expires_at" INTEGER,
    "token_type" TEXT,
    "scope" TEXT,
    "id_token" TEXT,
    "session_state" TEXT,

    CONSTRAINT "Account_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "Session" (
    "id" TEXT NOT NULL,
    "sessionToken" TEXT NOT NULL,
    "userId" TEXT NOT NULL,
    "expires" TIMESTAMP(3) NOT NULL,

    CONSTRAINT "Session_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "User" (
    "id" TEXT NOT NULL,
    "name" TEXT,
    "email" TEXT,
    "emailVerified" TIMESTAMP(3),
    "image" TEXT,

    CONSTRAINT "User_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "VerificationToken" (
    "identifier" TEXT NOT NULL,
    "token" TEXT NOT NULL,
    "expires" TIMESTAMP(3) NOT NULL
);

-- CreateTable
CREATE TABLE "Listing" (
    "id" TEXT NOT NULL,
    "title" TEXT NOT NULL,
    "website" TEXT NOT NULL,
    "category" TEXT NOT NULL,
    "size" TEXT NOT NULL,
    "city" TEXT NOT NULL,
    "neighborhood" TEXT NOT NULL,
    "latitude" DOUBLE PRECISION NOT NULL,
    "longitude" DOUBLE PRECISION NOT NULL,
    "twit" TEXT NOT NULL,
    "inst" TEXT NOT NULL,
    "linkATitle" TEXT NOT NULL,
    "linkAURL" TEXT NOT NULL,
    "linkBTitle" TEXT NOT NULL,
    "linkBURL" TEXT NOT NULL,
    "pullquote" TEXT NOT NULL,
    "episodePromo" TEXT NOT NULL,
    "episodePerson" TEXT NOT NULL,
    "episodeURL" TEXT NOT NULL,
    "color" TEXT NOT NULL,
    "published" BOOLEAN NOT NULL,

    CONSTRAINT "Listing_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "Tag" (
    "id" TEXT NOT NULL,
    "name" TEXT NOT NULL,

    CONSTRAINT "Tag_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "Directory" (
    "title" TEXT NOT NULL,
    "website" TEXT NOT NULL,
    "category" TEXT NOT NULL,
    "size" TEXT NOT NULL,
    "city" TEXT NOT NULL,
    "neighborhood" TEXT NOT NULL,
    "latitude" DOUBLE PRECISION NOT NULL,
    "longitude" DOUBLE PRECISION NOT NULL,
    "twit" TEXT NOT NULL,
    "inst" TEXT NOT NULL,
    "linkATitle" TEXT NOT NULL,
    "linkAURL" TEXT NOT NULL,
    "linkBTitle" TEXT NOT NULL,
    "linkBURL" TEXT NOT NULL,
    "pullquote" TEXT NOT NULL,
    "episodePromo" TEXT NOT NULL,
    "episodePerson" TEXT NOT NULL,
    "episodeURL" TEXT NOT NULL,
    "color" TEXT NOT NULL,
    "published" BOOLEAN NOT NULL,
    "three_dimensional" BOOLEAN NOT NULL,
    "advertising" BOOLEAN NOT NULL,
    "architecture" BOOLEAN NOT NULL,
    "branding" BOOLEAN NOT NULL,
    "community" BOOLEAN NOT NULL,
    "development" BOOLEAN NOT NULL,
    "eCommerce" BOOLEAN NOT NULL,
    "engineering" BOOLEAN NOT NULL,
    "environmental" BOOLEAN NOT NULL,
    "events" BOOLEAN NOT NULL,
    "exhibition" BOOLEAN NOT NULL,
    "experience" BOOLEAN NOT NULL,
    "experiential" BOOLEAN NOT NULL,
    "gaming" BOOLEAN NOT NULL,
    "illustration" BOOLEAN NOT NULL,
    "industrial" BOOLEAN NOT NULL,
    "inhouse" BOOLEAN NOT NULL,
    "interactive" BOOLEAN NOT NULL,
    "interior" BOOLEAN NOT NULL,
    "marketing" BOOLEAN NOT NULL,
    "motion" BOOLEAN NOT NULL,
    "naming" BOOLEAN NOT NULL,
    "packaging" BOOLEAN NOT NULL,
    "photography" BOOLEAN NOT NULL,
    "presentation" BOOLEAN NOT NULL,
    "press" BOOLEAN NOT NULL,
    "print" BOOLEAN NOT NULL,
    "printer" BOOLEAN NOT NULL,
    "product" BOOLEAN NOT NULL,
    "public_relations" BOOLEAN NOT NULL,
    "recruiter" BOOLEAN NOT NULL,
    "research" BOOLEAN NOT NULL,
    "school" BOOLEAN NOT NULL,
    "sound" BOOLEAN NOT NULL,
    "strategy" BOOLEAN NOT NULL,
    "typography" BOOLEAN NOT NULL,
    "ux_ui" BOOLEAN NOT NULL,
    "vfx" BOOLEAN NOT NULL,
    "video" BOOLEAN NOT NULL,
    "voice" BOOLEAN NOT NULL
);

-- CreateTable
CREATE TABLE "_TagToListing" (
    "A" TEXT NOT NULL,
    "B" TEXT NOT NULL
);

-- CreateIndex
CREATE INDEX "Account_userId_idx" ON "Account"("userId");

-- CreateIndex
CREATE UNIQUE INDEX "Account_provider_providerAccountId_key" ON "Account"("provider", "providerAccountId");

-- CreateIndex
CREATE UNIQUE INDEX "Session_sessionToken_key" ON "Session"("sessionToken");

-- CreateIndex
CREATE INDEX "Session_userId_idx" ON "Session"("userId");

-- CreateIndex
CREATE UNIQUE INDEX "User_email_key" ON "User"("email");

-- CreateIndex
CREATE UNIQUE INDEX "VerificationToken_token_key" ON "VerificationToken"("token");

-- CreateIndex
CREATE UNIQUE INDEX "VerificationToken_identifier_token_key" ON "VerificationToken"("identifier", "token");

-- CreateIndex
CREATE UNIQUE INDEX "Listing_title_key" ON "Listing"("title");

-- CreateIndex
CREATE UNIQUE INDEX "Tag_name_key" ON "Tag"("name");

-- CreateIndex
CREATE UNIQUE INDEX "Directory_title_key" ON "Directory"("title");

-- CreateIndex
CREATE UNIQUE INDEX "_TagToListing_AB_unique" ON "_TagToListing"("A", "B");

-- CreateIndex
CREATE INDEX "_TagToListing_B_index" ON "_TagToListing"("B");

-- AddForeignKey
ALTER TABLE "Account" ADD CONSTRAINT "Account_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Session" ADD CONSTRAINT "Session_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "_TagToListing" ADD CONSTRAINT "_TagToListing_A_fkey" FOREIGN KEY ("A") REFERENCES "Listing"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "_TagToListing" ADD CONSTRAINT "_TagToListing_B_fkey" FOREIGN KEY ("B") REFERENCES "Tag"("id") ON DELETE CASCADE ON UPDATE CASCADE;

