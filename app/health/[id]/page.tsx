...
export default async function ListingHealthPage({ params }: PageProps) {
  ...
  const { title, website, category, tags, description, published, healthChecks } = listingData;
      ...
      <div className="p-6 mb-8">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Tags: {tags.map((tag) => tag.name).join(", ")}</p>
            <p className="text-sm text-gray-500 mt-1">Description: {description || "No description available"}</p>
            <p className="text-sm text-gray-500 mt-1">Published: {published ? "Yes" : "No"}</p>
          </div>
